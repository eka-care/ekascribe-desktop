...
public partial class ProcessedOverlayWindow : Window
{
    ...
    private readonly ProcessedOverlayViewModel _viewModel;
    private const double PillCornerRadius = 16;

    internal ProcessedOverlayWindow(IOverlayStateStore overlayStateStore, IElectronBridgeClient? electronBridgeClient = null)
    {
        ...
        _viewModel = new ProcessedOverlayViewModel();
        _viewModel.DismissRequested += (_, _) =>
        {
            RequestHide(source: "overlay-ui");
        };
        _viewModel.ViewRequested += async (_, transactionId) =>
        {
            // Do NOT launch the ekadoc:// protocol here — on Windows that spawns a
            // second instance / relaunch and reloads the session tab. The Electron
            // scribe.result.view handler already brings an existing (minimized or
            // unfocused) window to front without reloading, and recreates it when
            // the window is closed. Mirrors macOS, which activates the host app
            // natively rather than via a protocol launch.
            try
            {
                if (_electronBridgeClient != null)
                {
                    await _electronBridgeClient.SendEventAsync(
                        "scribe.result.view",
                        new { transactionId = string.IsNullOrWhiteSpace(transactionId) ? null : transactionId },
                        CancellationToken.None);
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[OverlayUI] failed to send scribe.result.view: {ex.Message}");
            }
            RequestHide(source: "overlay-ui");
        };
    ...
    {
        // Ensure the overlay actually disappears even if the store is already "Hidden"
        // (in which case it may not emit a change event and the controller won't hide it).
    ...
    internal void ShowOverlayAnimated(string transactionId)
    {
        _isShown = true;
        _viewModel.SetTransactionId(transactionId);
